...
#
# worktree/ホスト環境からアクセス可能な状態管理。
# GitHub Issue のラベルとメタデータを使用。
#
# Usage: bash .pi/skills/github-issue-state-management/scripts/issue-state.sh <command> [args...]
#
# Commands:
...
#
# Example: bash .pi/skills/github-issue-state-management/scripts/issue-state.sh register 42 abc-123 feature/auth worktree
# ============================================================================
...
usage() {
    ...
    echo "      Register environment (env-type: worktree|host)"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  $0 register 42 abc-123 feature/auth worktree"
            ...
            "env:active")
                case "$PHASE" in
                    0-branch)
                        ...
                        echo "  \"command\": \"create branch ${BRANCH}\""
                        ;;
                    1-env|2-design|3-check|4-red|5-green|6-refactor)
                        echo "  \"action\": \"resume_work\","
                        echo "  \"command\": \"Checkout branch ${BRANCH} and continue work\""
                        ;;
                    7-review|8-stress)
                        echo "  \"action\": \"resume_review\","
                        echo "  \"command\": \"Continue review/stress test\""
                        ;;
                    9-approval)
