...
description: 各レビュアーエージェントが参照する詳細なレビュー観点（DB設計、セキュリティ、バックエンド、フロントエンド、インフラ）を定義
---
...
## 1. DB設計レビュー観点 (`database-reviewer` skill)
...
## 2. セキュリティレビュー観点 (`security-reviewer` skill)
...
## 3. バックエンドレビュー観点 (`backend-reviewer` skill)
...
## 4. フロントエンドレビュー観点 (`frontend-reviewer` skill)
...
## 5. インフラレビュー観点 (`infra-reviewer` skill)
