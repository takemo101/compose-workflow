...
- **既存提案書**: !`ls docs/proposals/PROP-*.md 2>/dev/null | wc -l | tr -d ' '` 件
- **improvement-proposalラベルのIssue**: !`gh issue list --label improvement-proposal --json number 2>/dev/null | jq 'length' || echo "0"` 件
