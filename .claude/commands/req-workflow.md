...
| 1 | 要件定義書作成 | `@req-writer` による作成 |
| 2 | レビューループ | `@req-reviewer` によるレビュー（8点以上、最大5回） |
...
4. **ユーザー確認（影響がある場合）**:
   ...
   **対応方針を選択してください**:
   ...
   1. 追記 → 既存要件定義書（REQ-XXX-001）に追記
   2. 新規 → 新規要件定義書（REQ-XXX-002）として作成
   3. 中断 → 確認後に再開
...
成果物サマリー出力後、以下の選択肢を提示してユーザーの応答を待つ。
...
| 回答 | アクション |
|-----|----------|
| `1` | `/basic-design-workflow "{成果物パス}"` を自動実行 |
