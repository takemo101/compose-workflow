...
4. **ユーザー選択**:
   ...
   **対応を選択してください**:
   ...
   1. 確認OK → 定義済み技術を承認し、未定義レイヤーの提案を確認
   2. 変更あり → 変更したい技術を指定してください
   3. 全て確定済み → 技術スタックは確定済み、確認不要で続行
...
**スキップ条件:**
- ユーザーが選択肢「3. 全て確定済み」を選択した場合のみ

> **注意**: 「要件定義書で定義済み」だけではスキップしない。必ずユーザーに確認を取る。
...
詳細設計フォルダ作成後、以下の選択肢を提示してユーザーの応答を待つ。
...
| 最終スコア | {score}/10点 |
| 成果物 | `{file_path}` |
| 詳細設計フォルダ | `docs/designs/detailed/{機能名}/` |
| ステータス | 合格 ✅ |
...
| 回答 | アクション |
|-----|----------|
| `1` | `/detailed-design-workflow "{成果物パス}"` を自動実行 |
| `2` | ユーザーの指摘を待ち、修正後に再レビュー |
| `3` | ワークフロー終了。成果物パスと次回再開方法を提示 |
