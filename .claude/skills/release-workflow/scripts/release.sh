...
full_release() {
    ...
    log_info "Step 1/4: Updating version..."
    update_version "$ecosystem" "$version" "$dry_run"
    
    # Step 2: Commit and tag
    log_info "Step 2/4: Committing and tagging..."
    commit_and_tag "$version" "$dry_run"
    
    # Step 3: Create GitHub release
    log_info "Step 3/4: Creating GitHub release..."
    create_github_release "$version" "" "$dry_run"
    
    # Step 4: Watch workflow
    if [[ "$dry_run" != "true" ]]; then
        log_info "Step 4/4: Watching release workflow..."
        watch_release_workflow
    else
        log_info "[DRY-RUN] Step 4/4: Would watch release workflow"
    fi
