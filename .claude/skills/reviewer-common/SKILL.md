...
description: 各レビュアーエージェント（backend, frontend, database, security, infra）の共通設定、動作モード、出力形式、スコアリング基準を定義
---
...
### Diff-Driven Review (Mode B)

> **Token最適化**: 実装レビュー時は、必ず「差分（Diff）」を最優先で確認する。
> 全文読み込みは、Diffだけでは文脈が不明な場合に限定する。

**手順:**

1. **Diff確認**: `git diff origin/main...HEAD` (または対象ブランチ間) を読む。
2. **影響範囲特定**: 変更されたファイルと行を特定。
3. **ピンポイント読み込み**: 文脈が必要な場合のみ、`read(offset, limit)` で該当箇所周辺を読む。
4. **禁止事項**: 変更がないファイルの全文読み込み、変更箇所以外の無意味な読み込み。

**プロンプト例:**

```
// Good: Diffベース
"Git Diffを確認し、変更されたロジック（AuthService.validate）のセキュリティを検証してください。"

// Bad: ファイル指定
"src/auth/auth.service.ts を読んで検証してください。"
```
