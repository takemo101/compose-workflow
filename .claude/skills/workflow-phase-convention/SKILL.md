...
|-------|-----------------|------------------|------|
| 0 | ブランチ作成 | バグ報告自動検出 | 準備・コンテキスト収集 |
| 0.5 | 設計書存在チェック | - | 設計書の有無を確認 |
| 0.6 | 設計書参照ルール | - | トークン最適化 |
| 1 | 環境構築 | Issue確認/作成 | container-use環境作成 |
| 1.5 | - | 環境選択 | container-use or worktree |
| 2 | 申し送り確認 | /implement-issues呼出 | 未完了事項の優先対応 |
| 3-5 | TDD実装 | - | Red → Green → Refactor |
| 6 | 設計不備対応 | - | `/request-design-fix`（必要時） |
| 6.5 | 実装完了自己チェック | - | TODO残存・Smoke Test・到達可能性 |
| 7 | 品質レビュー | - | スコア9点以上、客観的基準クリア |
| 7.1 | ユーザー承認 | - | 承認ゲート（`--auto`時スキップ） |
| 8 | コミット & プッシュ | - | git操作 |
