...
**技術選定の根拠となる公式情報のURLを確認します（詳細な調査は不要）。**

```
検索クエリ例（URL取得のみ）:
- "{技術名} official documentation"
- "{技術名} github repository"
```
...
**禁止事項:**
- 技術の詳細仕様を深掘り調査すること
- 複数の記事を比較してベストプラクティスを探すこと
- 実装方法を検索すること

**目的**: 提案書に「参考リンク」として記載するためだけの情報収集です。

---

### Phase H2: 技術スタック提案（提案形式ヒアリング）

調査結果（URL）を元に、以下の形式でユーザーに提案・確認：

**注意**: 詳細は提案時のリンク先を参照し、ユーザー自身が判断できるようにしてください。
詳細な比較情報が不足している場合は、推測で書かずに「要確認」としてください。
