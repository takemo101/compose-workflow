...
### Phase 2: クリティカル観点の調査（Web検索）

**検索は以下のクリティカルな観点のみに限定してください（最大2回まで）：**

| 優先度 | 観点 | 検索条件 |
|--------|------|----------|
| **必須** | セキュリティ・コンプライアンス | 認証/決済/個人情報を扱う機能の場合のみ |
| **必須** | 法的要件 | GDPR/PCI-DSS等が明らかに関係する場合のみ |
| 任意 | 業界標準 | ユーザーが明示的に調査を依頼した場合のみ |

**検索しない観点（LLMの事前学習知識で対応）：**
- 一般的な機能要件（CRUD、検索、通知など）
- よくあるユースケース・エッジケース
- 一般的なAPI設計パターン
- 基本的なベストプラクティス
...
```
検索クエリ（該当する場合のみ、最大2回）:
- "{機能名} security requirements 2024" （セキュリティ関連の場合）
- "{機能名} compliance GDPR PCI-DSS" （法規制関連の場合）
```

**重要**: 検索結果のリポジトリやドキュメントを深追いしない。検索結果のサマリーのみを活用すること。
